# --- SME OVERHAUL: Definitive, Fully Populated, and Corrected Version (PMA-Focused V4-Final) ---
"""
...
late-stage, PMA-track development program for a genomic diagnostic service
comprising both an IVD assay and a SaMD algorithm.
"""
import logging
import random
...
def _create_mced_diagnostic_dhf_model(version: int) -> Dict[str, Any]:
    ...
    # --- ML Data Generation ---
    num_samples = 100
    y_series = pd.Series(rng.integers(0, 2, num_samples))
    # Create a DataFrame with 10 features, 3 of which are highly predictive
    X_df = pd.DataFrame(rng.random((num_samples, 7)), columns=[f'noise_marker_{i}' for i in range(7)])
    X_df['promoter_A_met'] = y_series * 0.5 + rng.normal(0.2, 0.1, num_samples) # Predictive
    X_df['enhancer_B_met'] = y_series * 0.3 + rng.normal(0.1, 0.05, num_samples) # Predictive
    X_df['gene_X_expr'] = y_series * 0.4 + rng.normal(0.25, 0.1, num_samples) # Predictive
    X_df = X_df.clip(0, 1) # Ensure values are between 0 and 1
    ...
    return {
        ...
        "design_plan": {
            ...
            "standards": [
                {"id": "21 CFR 820", "title": "Quality System Regulation", "category": "Regulation"}, {"id": "ISO 13485:2016", "title": "Medical devices — Quality management systems", "category": "ISO Standard"},
                {"id": "ISO 14971:2019", "title": "Medical devices — Application of risk management", "category": "ISO Standard"}, {"id": "ISO 62304:2006", "title": "Medical device software — Software life cycle processes", "category": "ISO Standard"},
                {"id": "IEC 62366-1:2015", "title": "Application of usability engineering to medical devices", "category": "IEC Standard"}, {"id": "CLSI EP05-A3", "title": "Evaluation of Precision of Quantitative Measurement Procedures", "category": "CLSI Guideline"},
            ...
            "av_master_plan_ref": "AV-MP-001", "software_level_of_concern": "Major",
            "sw_dev_plan": "Software will be developed under a phase-gated Agile methodology, compliant with ISO 62304 Class C. All code will be managed in a version-controlled Git repository. Algorithm lock criteria are defined in SW-PLAN-001.",
            "risk_management_plan_ref": "RMP-001", "human_factors_plan_ref": "HFE-PLAN-001", "config_management_plan_ref": "CM-PLAN-001",
            "design_review_plan": [
        ...
        "risk_management_file": {
            ...
            "assay_fmea": [
                {"id": "AFM-01", "process_step": "Bisulfite Conversion", "failure_mode": "Incomplete DNA conversion", "potential_effect": "Incorrect methylation measurement, potential false result.", "S": 4, "O": 3, "D": 2, "RPN": 24, "mitigation": "Include unmethylated lambda DNA control in every run; set QC metric for conversion efficiency.", "verification_link": "AV-PREC-01"},
                {"id": "AFM-02", "process_step": "Library Amplification", "failure_mode": "PCR contamination", "potential_effect": "False positive signal due to foreign DNA.", "S": 5, "O": 2, "D": 3, "RPN": 30, "mitigation": "Use physically separated pre/post-PCR areas; negative controls in each batch.", "verification_link": "PPQ-01"},
                {"id": "AFM-03", "process_step": "Reagent Handling", "failure_mode": "Use of expired/improperly stored reagent", "potential_effect": "Systematic assay failure; inaccurate results for entire batch.", "S": 4, "O": 2, "D": 1, "RPN": 8, "mitigation": "LIMS-based reagent inventory tracking with automated expiry alerts.", "verification_link": "LIMS-VAL-01"}
            ],
            "service_fmea": [
                {"id": "SFM-01", "process_step": "Classifier Algorithm", "failure_mode": "Model overfitting to training data", "potential_effect": "Poor generalization, reduced accuracy on new patients.", "S": 5, "O": 3, "D": 3, "RPN": 45, "mitigation": "Use independent hold-out test set; cross-validation; L2 regularization.", "verification_link": "SW-VAL-01"},
                {"id": "SFM-02", "process_step": "Data Transfer", "failure_mode": "Corruption of FASTQ file during upload from sequencer", "potential_effect": "Run failure; potential for incorrect data analysis if corruption is subtle.", "S": 4, "O": 2, "D": 1, "RPN": 8, "mitigation": "Implement MD5 checksum verification for all file transfers.", "verification_link": "SW-VER-02"},
                {"id": "SFM-03", "process_step": "Report Generation", "failure_mode": "Patient demographic data mismatched with result data", "potential_effect": "Correct result delivered on a report with wrong patient name.", "S": 5, "O": 1, "D": 2, "RPN": 10, "mitigation": "Automated data merge using unique sample ID as primary key; manual QC check of report before release.", "verification_link": "LIMS-VAL-01"}
            ],
        ...
        "human_factors": {
            "use_specification": {"intended_use_profile": "Screening for early cancer signal detection in asymptomatic, average-risk individuals aged 50-79.", "intended_user_profile": "Phlebotomists, Laboratory Technologists, Oncologists/PCPs.", "intended_use_environment": "Outpatient clinics, clinical laboratories, physician offices.", "key_operational_functions": "Sample collection, sample processing, data analysis, result interpretation."},
            "user_profiles": [{"user_group": "Phlebotomist", "description": "Certified phlebotomist with >1 year experience in clinical sample collection.", "training": "Standard phlebotomy training + product-specific IFU.", "use_environment": "Outpatient clinic or patient service center."}, {"user_group": "Lab Technologist", "description": "ASCP-certified technologist with experience in molecular diagnostics.", "training": "BS in a chemical/physical/biological science + SOP-specific training.", "use_environment": "CLIA-certified, high-complexity molecular diagnostics laboratory."}, {"user_group": "Oncologist", "description": "Board-certified oncologist responsible for patient management.", "training": "Medical degree, specialty training.", "use_environment": "Hospital or outpatient clinic office."}],
        ...
        "design_inputs": { "requirements": [
                {"id": "UN-001", "type": "User Need", "description": "Provide a definitive result (Cancer Signal Detected/Not Detected) to guide patient management.", "parent_id": "", "status": "Active", "is_risk_control": False, "related_hazard_id": ""}, {"id": "UN-002", "type": "User Need", "description": "The process of collecting a sample should be simple and minimally invasive.", "parent_id": "", "status": "Active", "is_risk_control": False, "related_hazard_id": ""},
                {"id": "SYS-001", "type": "System", "description": "The test shall achieve >99.0% specificity across all cancer types in the intended use population.", "parent_id": "UN-001", "status": "Active", "is_risk_control": True, "related_hazard_id": "H-02"}, {"id": "SYS-002", "type": "System", "description": "The test shall achieve >85.0% sensitivity for Stage III cancers for the top 5 cancer types.", "parent_id": "UN-001", "status": "Active", "is_risk_control": True, "related_hazard_id": "H-01"},
                {"id": "ASY-001", "type": "Assay", "description": "The assay shall achieve a Limit of Detection (LoD) of <=0.1% tumor fraction for key biomarkers.", "parent_id": "SYS-002", "status": "Active", "is_risk_control": True, "related_hazard_id": "H-01"}, {"id": "ASY-002", "type": "Assay", "description": "The inter-assay precision for a 0.5% TF control shall be <=20% CV.", "parent_id": "SYS-001", "status": "Active", "is_risk_control": False, "related_hazard_id": ""},
                {"id": "SW-001", "type": "Software", "description": "The bioinformatics pipeline shall align >95% of reads to the hg38 reference genome.", "parent_id": "SYS-001", "status": "Active", "is_risk_control": False, "related_hazard_id": ""}, {"id": "SW-002", "type": "Software", "description": "The system shall verify file integrity of all data transfers using a checksum.", "parent_id": "SYS-001", "status": "Active", "is_risk_control": True, "related_hazard_id": "H-03"},
        ...
        "design_outputs": { "documents": [
                {"id": "SPEC-ASY-01", "type": "Assay Spec", "title": "Oligonucleotide Probe Sequences v1.2", "version": "1.2", "status": "Approved", "approval_date": str(base_date + timedelta(days=100)), "linked_input_id": "ASY-001", "link_to_artifact": "/docs/SPEC-ASY-01_v1.2.pdf"}, {"id": "SPEC-SW-01", "type": "Software Spec", "title": "Bioinformatics Pipeline Software Design Specification (SDS)", "version": "2.1", "status": "Approved", "approval_date": str(base_date + timedelta(days=250)), "linked_input_id": "SW-001", "link_to_artifact": "https://github.com/genomicsdx/pipeline/docs/sds_v2.1.md"},
                {"id": "MODEL-01", "type": "Algorithm", "title": "Locked Classifier Model v3.0", "version": "3.0", "status": "Approved", "approval_date": str(base_date + timedelta(days=365)), "linked_input_id": "SYS-001", "link_to_artifact": "/models/classifier_v3.0.pkl"}, {"id": "PROC-LAB-01", "type": "Procedure", "title": "SOP for cfDNA Extraction and Library Prep", "version": "2.0", "status": "In Review", "approval_date": None, "linked_input_id": "ASY-001", "link_to_artifact": None},
        ...
        "design_reviews": { "reviews": [
                {"id": "DR-001", "date": str(demo_current_date - timedelta(days=270)), "attendees": team_list, "type": "Assay", "phase": "Assay Concept", "independent_reviewer": "Jose Bautista", "scope": "Initial review of assay chemistry and biomarker panel.", "outcome": "Go", "notes": "Concept review complete. Feasibility approved.", "action_items": [{"id": "AI-DR001-01", "description": "Source alternative polymerase for improved stability.", "owner": "Sofia Chen, PhD", "due_date": str(demo_current_date - timedelta(days=255)), "status": "Completed", "risk_priority": "Medium", "voe_status": "N/A"}]},
                {"id": "DR-002", "date": str(base_date + timedelta(days=365)), "attendees": team_list, "type": "Phase-Gate", "phase": "Algorithm Lock", "independent_reviewer": "Sofia Chen, PhD", "scope": "Review of classifier performance on training data. Approval to lock model for clinical validation.", "outcome": "Go with Conditions", "notes": "Performance is acceptable. Condition: Finalize Software V&V report before starting patient enrollment.", "action_items": [{"id": "AI-DR003-01", "description": "Complete and approve SW-VAL-01.", "owner": "Marcus Thorne, PhD", "due_date": str(base_date + timedelta(days=380)), "status": "In Progress", "risk_priority": "High", "voe_status": "N/A"}]}
            ]},
        "design_verification": { "tests": [
                {"id": "AV-LOD-01", "test_type": "Limit of Detection (LoD)", "test_name": "LoD Study by Probit Analysis using Contrived Samples", "input_verified_id": "ASY-001", "output_verified_id": "SPEC-ASY-01", "status": "Completed", "result": "Pass", "report_link": "/reports/AV-LOD-01.pdf"}, {"id": "AV-PREC-01", "test_type": "Intermediate Precision", "test_name": "Precision Study across 3 operators, 3 lots, 5 days", "input_verified_id": "ASY-002", "output_verified_id": "SPEC-ASY-01", "status": "Completed", "result": "Pass", "report_link": "/reports/AV-PREC-01.pdf"},
                {"id": "SW-VER-01", "test_type": "SW Verification", "test_name": "Unit Test Suite for Alignment Module", "input_verified_id": "SW-001", "output_verified_id": "SPEC-SW-01", "status": "Completed", "result": "Pass", "report_link": "/reports/SW-VER-01.pdf"}, {"id": "SW-VER-02", "test_type": "SW Verification", "test_name": "Verification of MD5 Checksum Implementation", "input_verified_id": "SW-002", "output_verified_id": "SPEC-SW-01", "status": "Completed", "result": "Pass", "report_link": "/reports/SW-VER-02.pdf"},
            ]},
        "clinical_study": {
            "nct_id": "NCT08675309", "phase": "Pivotal", "protocol_id": "CSP-001", "primary_endpoints": "Co-primary endpoints are sensitivity and specificity for the detection of a cancer signal.",
            "enrollment": [{"site": "Site-01 (USA)", "target": 5000, "enrolled": 3102}, {"site": "Site-02 (USA)", "target": 5000, "enrolled": 2895}],
            "performance_metrics": [{"metric": "Overall Sensitivity", "value": "N/A (Interim)", "confidence_interval": "N/A"}, {"metric": "Overall Specificity", "value": "99.5% (Interim)", "confidence_interval": "99.1% - 99.8%"}],
            "hf_studies": [{"id": "HF-VAL-01", "study_name": "Summative Usability Study - Sample Collection Kit", "user_interface_validated": "Sample Collection Kit & IFU", "user_need_validated": "UN-002", "confirms_risk_control": True, "status": "Completed", "report_link": "/reports/HF-VAL-01.pdf"}, {"id": "HF-VAL-02", "study_name": "Summative Usability Study - Clinical Report Interpretation", "user_interface_validated": "Clinical Report", "user_need_validated": "UN-001", "confirms_risk_control": True, "status": "In Progress", "report_link": ""}],
            "adverse_events": [{"id": "AE-001", "site": "Site-01 (USA)", "description": "Bruising at venipuncture site", "severity": "Mild", "related_to_device": True}]
        },
        "lab_operations": {
            "sops": [{"doc_id": "SOP-LAB-001", "title": "Sample Accessioning and LIMS Entry", "version": "1.2", "status": "Approved"}, {"doc_id": "SOP-LAB-002", "title": "cfDNA Extraction", "version": "1.0", "status": "Approved"}, {"doc_id": "SOP-LAB-003", "title": "Library Preparation", "version": "0.9", "status": "In Review"}],
            "infrastructure": [{"asset_id": "SEQ-01", "equipment_type": "Illumina NovaSeq X", "status": "PQ Complete"}, {"asset_id": "LIMS-01", "equipment_type": "LabVantage LIMS v8", "status": "PQ Complete"}],
            "software_deployment": [{"component": "Classifier Model", "version": "3.0", "deployment_date": str(base_date + timedelta(days=366)), "validation_protocol": "SW-DEP-VAL-01"}],
            "ppq_runs": [{"run_id": "PPQ-01", "description": "Run 1 of 3", "run_date": str(demo_current_date - timedelta(days=20)), "result": "Pass"}, {"run_id": "PPQ-02", "description": "Run 2 of 3", "run_date": str(demo_current_date - timedelta(days=10)), "result": "Pass"}, {"run_id": "PPQ-03", "description": "Run 3 of 3", "run_date": str(demo_current_date - timedelta(days=1)), "result": "In Progress"}],
            ...
            "readiness": {"reagent_lot_qualification": {"total": 10, "passed": 9}, "inter_assay_precision": {"cv_pct": 8.5, "target_cv": 15.0}, "sample_stability_studies": [{"condition": "7 Days Ambient", "result": "Pass"}, {"condition": "3 Freeze/Thaw Cycles", "result": "Pass"}]}
        },
        "quality_system": {
            "capa_records": [{"id": "CAPA-02", "status": "Open", "source": "Supplier Corrective Action for IDT-Lot-123", "description": "High rate of oligo synthesis failure in a critical reagent lot from supplier IDT.", "action_plan": [{"id": "AI-CAPA02-01", "description": "Qualify new reagent lot from alt supplier", "owner": "Sofia Chen, PhD", "due_date": str(demo_current_date + timedelta(days=30)), "status": "Open", "risk_priority": "High", "voe_status": "N/A"}]}],
            "ncr_records": [], "supplier_audits": [{"supplier": "IDT", "status": "Pass with Observations"}, {"supplier": "Illumina", "status": "Pass"}],
            ...
            "hypothesis_testing_data": {'pipeline_a': list(rng.normal(0.012, 0.005, 30)), 'pipeline_b': list(rng.normal(0.010, 0.005, 30))},
            "equivalence_data": {'reagent_lot_a': list(rng.normal(0.85, 0.05, 30)), 'reagent_lot_b': list(rng.normal(0.86, 0.05, 30))},
            ...
            "lod_probit_data": [{"Concentration": 0.01, "Hit Rate": 0.05}, {"Concentration": 0.05, "Hit Rate": 0.40}, {"Concentration": 0.1, "Hit Rate": 0.85}, {"Concentration": 0.15, "Hit Rate": 0.95}, {"Concentration": 0.2, "Hit Rate": 1.0}],
            "interference_data": {'Baseline_Value': rng.normal(10, 1.5, 20), 'Interferent_Value': rng.normal(10.2, 1.5, 20)},
            "survival_data": [{'time': t, 'event': e, 'cohort': c} for t, e, c in zip(list(rng.exponential(365, 50)) + list(rng.exponential(900, 50)), list(rng.integers(0, 2, 100)), ['Signal Detected']*50 + ['No Signal Detected']*50)],
        },
        ...
        "ml_models": {
            ...
            "sample_volume_data": pd.DataFrame({'date': pd.to_datetime(pd.date_range(start=base_date, periods=180, freq='D')),'samples': [int(10 + (i/10) + gauss(0, i/20)) for i in range(180)]}).to_dict('records'),
            "data_drift_data": {'training_dist': X_df['promoter_A_met'].values, 'production_dist': (X_df['promoter_A_met'] + rng.normal(0.05, 0.01, num_samples)).values}
        }
    }
...
class SessionStateManager:
    ...
    def __init__(self):
        ...
        if not session_data or session_data.get("data_version") != self._CURRENT_DATA_VERSION:
            ...
            ### BUG FIX: Ensure model is trained on the full feature set
